...
def get_dist_restraints_hydrophobe(filename, s, scaler, ramp, seq):
    ...
    for line in lines:
        if not line:
            ...
            #==================== debug ================+++++++++++++++++++++++++
            #if i < 0 or i >= len(seq):
            #    raise ValueError(f"Atom index {i} out of bounds for sequence length {len(seq)}")
            #if j < 0 or j >= len(seq):
            #    raise ValueError(f"Atom index {j} out of bounds for sequence length {len(seq)}")

            #residues = list(s.topology.residues())
            #res_atoms = [atom.name for atom in residues[i].atoms()]
            #if name_i not in res_atoms:
            #    raise ValueError(f"Atom name '{name_i}' not found in residue {seq[i]} at index {i}")
            #res_atoms_j = [atom.name for atom in residues[j].atoms()]
            #if name_j not in res_atoms_j:
            #    raise ValueError(f"Atom name '{name_j}' not found in residue {seq[j]} at index {j}")

            # Print atom indices and check bounds
            #atom1_idx = s.index.atom(i, name_i, expected_resname=seq[i][-3:])
            #atom2_idx = s.index.atom(j, name_j, expected_resname=seq[j][-3:])
            #print(f"Restraint: residue {i+1} atom {name_i} (atom idx {atom1_idx}), residue {j+1} atom {name_j} (atom idx {atom2_idx})")
            #num_atoms = len(list(s.topology.atoms()))
            #if atom1_idx < 0 or atom2_idx < 0:
            #    raise ValueError(f"Invalid atom index: {atom1_idx} or {atom2_idx}")
            #if atom1_idx >= num_atoms or atom2_idx >= num_atoms:
            #    raise ValueError(f"Atom index out of bounds: {atom1_idx} or {atom2_idx} (num_atoms={num_atoms})")
            #========================================================================
...
def setup_system():
    ...
    #======================= newly added line tp setup the openmm topology =========================
    ...
    #import numpy as np
    #coords = np.array([list(pos.value_in_unit(unit.nanometer)) for pos in modeller.positions])
    #print("Any NaNs in coordinates?", np.isnan(coords).any())
    #if np.isnan(coords).any():
    #    nan_indices = np.where(np.isnan(coords).any(axis=1))[0]
    #    print("NaN at atom indices:", nan_indices)
    #    for idx in nan_indices:
    #        atom = list(modeller.topology.atoms())[idx]
    #        print(f"Atom {idx}: {atom.name} in residue {atom.residue}")
    #    raise RuntimeError("NaN in coordinates. Fix your PDB or system setup.")
    # === End of NaN check ===
    ...
    grappa_options = GrappaOptions(
        ...
        cutoff=None,
        use_big_timestep=False # CRITICAL: Set to False
    )
